...
def get_col(planilha, coluna):
    ...
    sheet = gc.open_by_url(
        'https://docs.google.com/spreadsheets/d/(...ocultado...)'
        )
    ...
    if WORKFLOW == 'LOCAL':
        ...
        for numero in Processos:
            ...
            try:
                ...
                ProcessoErro = ProcessoErro.append(
                    {'NumeroProcessoErro': numero, 'NumeroProcessoErroCompleto': NumProcCompleto, 'TipoErro': 'TypeError'}, ignore_index=True)
                pass

            # Grava localmente a saída dos arquivos
            DadosProcesso.to_csv('DadosProcessoGoogle2.csv',
                                 encoding='Latin1', sep=';')
            ProcessoErro.to_csv('ListaErroOutGoogle2.csv',
                                encoding='Latin1', sep=';')
